    ...
    fieldSchemas.push([
      ...
      Object.keys(add).map(field => {
        ...
        if(add[field].createType){
          ...
          nestedSchemas.push(`type ${add[field].type} {${EOL}${objectToSchema(add[field])}${EOL}}`);
         }
...
function objectToSchema(object){
  ...
  Object.keys(properties).map((name)=>{
    ...
    let array = false;
       ...
       if (type === 'array') {
        ...
        if (typeof items === 'object') {
          ...
          if(items.type === 'object' && items.createType && items.properties){
            nestedSchemas.push(`# ${items.description || `No description available`}`);
            nestedSchemas.push(`type ${items.createType} {${EOL}${objectToSchema(items)}${EOL}}`);
          }
          array = true;
        } else {
    ...
    return scalarTypes.push(`${name}: ${ array ? '[' + schemaTypeToGraphql[type] + ']' : schemaTypeToGraphql[type] }`);
  })
...
function feathersSchemaToGraphqlSchema (feathersSchema, feathersExtension, depth = 1) {
  ...
  const graphqlTypes = Object.keys(properties).map(name => {
    ...
    if (type === 'object') {
      if(property.createType && property.properties  ){
         nestedSchemas.push(`# ${property.description ? 
          property.description : 'JSON'}${EOL}type ${property.createType} {${EOL}${objectToSchema(property)}${EOL}}`);
      }
      return scalarTypes.push(`${name}: ${property.createType ? property.createType : schemaTypeToGraphql.object}`);
    }
