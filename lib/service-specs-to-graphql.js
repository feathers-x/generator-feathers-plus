...
const EOL = '\n';
const nestedSchemas = []; // Nested Object Types
...
function objectToSchema(object){
 ...
 return scalarTypes.filter(prop => prop).join(`${EOL}${leader}`);
}
  ...
  const graphqlTypes = Object.keys(properties).map(name => {
    ...
    if (type === 'object') {
      if(property.__schema){
         nestedSchemas.push({
          type: `${EOL}type ${property.__schema} {${EOL}${objectToSchema(property)}${EOL}}${EOL}`
        });
      }
