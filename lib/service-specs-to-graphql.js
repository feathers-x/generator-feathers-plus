...
function feathersSchemaToGraphqlSchema (feathersSchema, feathersExtension, depth = 1) {
  ...
  const graphqlTypes = Object.keys(properties).map(name => {
    ...
    if (type === 'object') {
      if(property.__addType){
         nestedSchemas.push(`# ${property.description ? 
          property.description : 'JSON'}${EOL}type ${property.__addType} {${EOL}${objectToSchema(property)}${EOL}}`);
      }
      return scalarTypes.push(`${name}: ${property.__addType ? property.__addType : schemaTypeToGraphql.object}`);
    }
    ...
    if (type === 'array') {
      ...
      if (typeof items === 'object') {
        ...
        if(items.type === 'object' && items.__addType){
    ...
    scalarTypes.push(`${name}: ${array ? '[' : ''}${!items.graphType ? schemaTypeToGraphql[type.trim()] : items.graphType}${req ? '!' : ''}${array ? ']' : ''}`);
