    ...
    fieldSchemas.push([
      ...
      Object.keys(add).map(field => {
        ...
        if (add[field].description) {
          ...
          if(add[field].addType){
            fieldSchemas.push(`# ${add[field].description ? 
              add[field].description : 'JSON'}${EOL}type ${add[field].addType} {${EOL}${objectToSchema(add[field].properties)}${EOL}}`);
         }}
