...
module.exports = function serviceSpecsToSequelize (feathersSpec, feathersExtension) {
  ...
  Object.keys(obj.properties).forEach(prop => {
    if (obj.properties[prop].anyOf) {
      let type = _.intersection(obj.properties[prop].anyOf, ['string', 'number', 'boolean', 'object', 'array']).pop();
      obj.properties[prop] = {type, ...obj.properties[prop]};
    }
  });
  ...
  Object.keys(seqModel).forEach(fieldName => {
    ...
    if (fieldModel.description) delete fieldModel.description;
