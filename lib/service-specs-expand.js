...
function expandProperties (feathersSchema, depth = 1) {
  ...
  Object.keys(properties || {}).forEach(fieldName => {
    ...
    if (property.anyOf) {
      // property.anyOf = property.anyOf.map(e=>e.type !== 'null' ? e.type : '')
      property.type = _.intersection(property.anyOf.map(e => e.type !== 'null' ? e.type : ''), ['string', 'number', 'boolean', 'object', 'array']).pop();
    }

    if (property.type instanceof Array) {
      property.type = _.intersection(property.type, ['string', 'number', 'boolean', 'object', 'array']).pop();
    }
