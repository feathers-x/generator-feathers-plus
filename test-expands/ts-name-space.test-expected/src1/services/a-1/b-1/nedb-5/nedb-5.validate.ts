...
let quickValidate = (method: string, data: any, options?: any) => {
  try {
    ...
    if (method === 'update') { validateUpdate(options)({ type: 'before', method: 'update', data } as any); }
    if (method === 'patch') { validatePatch(options)({ type: 'before', method: 'patch', data } as any); }
  } catch (err) {
    return err;
  }
};
